...
void copyData(std::vector<Read> &reads, Data &data) {
    ...
    hipMallocManaged(&data.lengths, readsCount*sizeof(int));
    hipMallocManaged(&data.offsets, (readsCount+1)*sizeof(long));
    data.offsets[0] = 0;
    ...
    hipMallocManaged(&data.reads, data.offsets[readsCount]*sizeof(char));
...
void baseToNumber(Data &data) {
    ...
    hipLaunchKernelGGL(kernel_baseToNumber, dim3(128), dim3(128), 0, 0, data.reads, length);
    hipDeviceSynchronize();  // 同步数据
}
// 压缩后每个碱基占两个bit，去掉gap
...
void compressData(Data &data) {
    ...
    hipMallocManaged(&data.compressed, length/16*sizeof(unsigned int));
    hipMallocManaged(&data.gaps, readsCount*sizeof(int));
    hipLaunchKernelGGL(kernel_compressData, dim3((readsCount+127)/128), dim3(128), 0, 0, data.lengths,
...
void createIndex(Data &data, Option &option) {
    ...
    hipMallocManaged(&data.indexs, length*sizeof(unsigned short));  // index值
    hipMallocManaged(&data.orders, length*sizeof(unsigned short));  // index秩
    hipMallocManaged(&data.words, (readsCount+1)*sizeof(long));  // index长度
    hipMallocManaged(&data.magicBase, readsCount*4*sizeof(int));  // 魔法碱基
    switch (wordLength) {
        case 4:
            hipLaunchKernelGGL(kernel_createIndex4, dim3((readsCount+127)/128), dim3(128), 0, 0, data.reads, data.lengths, data.offsets,
            data.indexs, data.orders, data.words, data.magicBase, readsCount);
            break;
        case 5:
            hipLaunchKernelGGL(kernel_createIndex5, dim3((readsCount+127)/128), dim3(128), 0, 0, data.reads, data.lengths, data.offsets,
            data.indexs, data.orders, data.words, data.magicBase, readsCount);
            break;
        case 6:
            hipLaunchKernelGGL(kernel_createIndex6, dim3((readsCount+127)/128), dim3(128), 0, 0, data.reads, data.lengths, data.offsets,
            data.indexs, data.orders, data.words, data.magicBase, readsCount);
            break;
        case 7:
            hipLaunchKernelGGL(kernel_createIndex7, dim3((readsCount+127)/128), dim3(128), 0, 0, data.reads, data.lengths, data.offsets,
            data.indexs, data.orders, data.words, data.magicBase, readsCount);
            break;
    }
    hipDeviceSynchronize();  // 同步数据
...
void createCutoff(Data &data, Option &option) {
    int readsCount = data.readsCount;
    hipMallocManaged(&data.wordCutoff, readsCount*sizeof(int));  // word阈值
    hipLaunchKernelGGL(kernel_createCutoff, dim3((readsCount+127)/128), dim3(128), 0, 0, option.threshold, option.wordLength, data.lengths,
...
void mergeIndex(Data &data) {
    int readsCount = data.readsCount;
    hipLaunchKernelGGL(kernel_mergeIndex, dim3((readsCount+127)/128), dim3(128), 0, 0, data.offsets, data.indexs, data.orders, data.words, readsCount);
    hipDeviceSynchronize();  // 同步数据
}
...
void clustering(Option &option, Data &data, Bench &bench) {
    int readsCount = data.readsCount;
    hipMallocManaged(&bench.cluster, readsCount*sizeof(int));  // 聚类结果
    for (int i=0; i<readsCount; i++) {
        bench.cluster[i] = -1;
    }
    hipMallocManaged(&bench.table, 65536*sizeof(unsigned short));  // table
...
hipDeviceSynchronize();  // 同步数据
        hipLaunchKernelGGL(kernel_makeTable, dim3(128), dim3(128), 0, 0, data.offsets, data.indexs, data.orders, data.words,
        bench.table, bench.representative);  // 生成table
hipDeviceSynchronize();  // 同步数据
        hipLaunchKernelGGL(kernel_magic, dim3((readsCount+127)/128), dim3(128), 0, 0, option.threshold, data.lengths, data.magicBase,
        bench.cluster, bench.representative,readsCount);  // 魔法过滤
hipDeviceSynchronize();  // 同步数据
        hipLaunchKernelGGL(kernel_filter, dim3(readsCount), dim3(128), 0, 0, option.threshold, option.wordLength, data.lengths,
        data.offsets, data.indexs, data.orders, data.words,
        data.wordCutoff, bench.cluster, bench.table, readsCount);  // word过滤
hipDeviceSynchronize();  // 同步数据
        hipLaunchKernelGGL(kernel_align, dim3((readsCount+127)/128), dim3(128), 0, 0, option.threshold, data.lengths, data.offsets, data.compressed,
        data.gaps, bench.representative, bench.cluster, readsCount, 3000);  // 比对
hipDeviceSynchronize();  // 同步数据
        hipLaunchKernelGGL(kernel_cleanTable, dim3(128), dim3(128), 0, 0, data.offsets, data.indexs, data.orders, data.words,
